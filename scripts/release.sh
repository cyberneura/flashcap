...
# 明示しておけば remote の fetch 設定に依存せず必ず最新と比較できる。
# 先頭の + は clone 既定の refspec と同じ強制更新。force push 後も fetch 自体は
# 成功させ、ズレは下の HEAD 比較で検出する。
...
# 終わるのを避ける)。JSON をパースしてトップレベルの version 値を取得し、その値を
# 持つ最初の "version" 行だけを置換する (ファイル全体を再整形しないため diff が
# version 1 行に収まる)。両ファイルともトップレベル version が先頭付近にあるので
# 誤爆しないが、将来同じ値の nested な version キーがそれより前に来ると当たる。
node -e '
  const fs = require("fs");
...
# version は毎回インクリメントされるので、この SHA を持つ run は今回の1つだけ。
...
# 60 回 x 2 秒 = 最大 2 分待つ。run は通常数秒で現れるが、GitHub の run 一覧 API が
# 遅れることがあり、短い上限だと「起動はしているのに見つからない」と誤判定する。
RUN_ID=""
...
echo "Done: https://github.com/ytyng/flashcap/releases/tag/v${VERSION}"
